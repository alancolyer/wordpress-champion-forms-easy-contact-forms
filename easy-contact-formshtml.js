...
var ufoCf = new function(){
	...
	this.setActiveField = function(hash, id){
		...
		ufoCf.curentItem = $li;
		ufoCf.getFieldSettingsForm(hash, id); 
		this.jqInit();
	} 
 
	this.getFieldSettingsForm = function(hash, fid){
		if (typeof(hash) == 'object'){
			hash = AppMan.Utils.idSplit(hash.id)[0]; 
		}
	...
	this.refreshForm = function(el, formid)	{
		...
		config.m = 'refreshForm';
		config.oid = formid;
		var ajx = {};
		ajx.params = config;
		ajx.success = function(data) {
			var resp = AppMan.JSON.decode(data);
			jQuery(el).removeClass('ufo-loading');	
			jQuery('#'+target).attr('src', resp.url);								
